...
from crud.column_constraint import create_constraint, get_constraint, update_constraint, delete_constraint, list_constraints
...
def create_constraint_route(constraint_data: ConstraintCreate, db: Session = Depends(get_db)):
...
def update_constraint_route(constraint_id: int, constraint_data: ConstraintUpdate, db: Session = Depends(get_db)):
    constraint = get_constraint(db, constraint_id)
    if not constraint:
        raise HTTPException(status_code=404, detail="Constraint not found")
