...
class TableConfiguration(SQLModel, table=True):
    ...
    years_to_collect: int = Field(description="Integer, how many years of data we wish to collect (number of rows to render).")
    created_by: int = Field(
        foreign_key="user.id", description="User ID of the creator."
    )
    updated_by: int = Field(
        default=None,
        foreign_key="user.id", description="User ID of the last person to update."
    )
